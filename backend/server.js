...
app.post("/groups", requiredLogin, async (req, res) => {
    ...
    try{
        const query = `
        INSERT INTO groups (game, title, description, max_players, creator_username, tags)
        VALUES ($1, $2, $3, $4, $5, $6)
        RETURNING id, title, game, description, max_players, current_players, creator_username, tags
        `;
        const values = [game.trim(), title.trim(), description.trim(), parseInt(max_players), creator_username, safeTags];
        const result = await db.query(query, values);
